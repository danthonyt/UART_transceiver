module fifo_ctrl_fsm #(
    ...
    input  logic        tx_fifo_rstn  ,

    // rx fifo control
    output logic        rx_fifo_wen  ,
    input  logic        rx_fifo_full ,
    input  logic        rx_fifo_rstn  ,
    ...
    assign nxt_tx_start       = (tx_fifo_rstn && tx_fifo_ren && !tx_busy) ? 1 : 0;

    // only read from tx fifo if it is not empty, not in reset, and not already transmitting
    assign nxt_tx_fifo_ren    = (!tx_fifo_empty && tx_fifo_rstn && !tx_busy && !tx_fifo_ren) ? 1 : 0;

    assign nxt_rx_fifo_wdata  = rx_byte;

    // only write to rx fifo if byte is valid, fifo is not full, and fifo is not in reset
    assign nxt_rx_fifo_wen    = (rx_byte_valid && !rx_fifo_full && rx_fifo_rstn) ? 1 : 0;
